...
const MODE_I18N: Record<string, [string, string]> = {
  ...
  'business-brainstorm': ['businessBrainstorm', 'businessBrainstormDesc'],
  'comedy-duo': ['comedyDuo', 'comedyDuoDesc'],
  'love-advice': ['loveAdvice', 'loveAdviceDesc'],
  'pro-brainstorm': ['proBrainstorm', 'proBrainstormDesc'],
};
    ...
    <div className="fixed inset-0 z-[60] animate-fadeIn" onClick={onClose}>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-slate-900 rounded-t-3xl p-5 pb-10" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-white font-bold text-lg">{t.customCreate}</h3>
        ...
        <div className="space-y-4">
          <div>
            <label className="text-white/60 text-xs font-bold uppercase tracking-wider mb-1 block">{t.charName}</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder={t.charNamePlaceholder} className="w-full px-4 py-3 bg-white/10 rounded-xl text-white placeholder-white/20 border border-white/10 focus:border-white/30 focus:outline-none" />
          </div>
          <div>
            <label className="text-white/60 text-xs font-bold uppercase tracking-wider mb-1 block">{t.charPersonality} <span className="text-white/30 font-normal normal-case">{t.charPersonalityOptional}</span></label>
            <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder={t.charPersonalityPlaceholder} rows={3} className="w-full px-4 py-3 bg-white/10 rounded-xl text-white placeholder-white/20 border border-white/10 focus:border-white/30 focus:outline-none resize-none" />
          </div>
    ...
    <button onClick={() => setModal(slot)} className="flex-1 h-[160px] rounded-3xl border-2 border-dashed border-white/20 flex flex-col items-center justify-center gap-3 transition-all duration-300 cursor-pointer active:scale-95 hover:border-white/40 overflow-hidden relative">
      {char ? (
        ...
        <>
          ...
          <span className="text-white/30 text-xs font-medium">{lang === 'ja' ? 'タップして選択' : 'Tap to select'}</span>
        </>
      )}
    </button>
  );
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-violet-950 via-indigo-950 to-slate-950 overflow-y-auto scrollbar-hide">
      ...
      <div className="flex-1 px-5 pb-5 space-y-6 animate-slideUp">
        ...
        {/* Mode */}
        <div>
          <h3 className="text-white/60 text-xs font-bold uppercase tracking-wider mb-3">{t.selectMode}</h3>
          <div className="space-y-2">
            {modes.map((m, i) => { const MIcon = MODE_ICONS[m.id] || Sparkles; const sel = selMode?.id === m.id; const isPro = PRO_MODES.has(m.id); const isUnlocked = typeof window !== 'undefined' && !!new URLSearchParams(window.location.search).get('session_id'); return (
              <button key={m.id} onClick={async () => { if (isPro && !isUnlocked) { try { const res = await fetch('/api/checkout', { method: 'POST' }); const data = await res.json(); if (data.url) window.location.href = data.url; } catch { alert('Checkout unavailable'); } } else { setSelMode(m); } }} className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all duration-200 cursor-pointer active:scale-[0.98] ${sel ? 'bg-white/15 border border-white/30 shadow-lg shadow-purple-500/10' : isPro ? 'bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/30 hover:border-amber-500/50' : 'bg-white/5 border border-transparent hover:bg-white/10'}`}>
                ...
                <div className="text-left flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`font-bold text-sm ${sel ? 'text-white' : isPro ? 'text-amber-300' : 'text-white/70'}`}>{MODE_I18N[m.id] ? (t as any)[MODE_I18N[m.id][0]] : m.name}</span>
                    {isPro && <span className="text-[9px] font-bold bg-amber-500 text-black px-1.5 py-0.5 rounded-full">$1</span>}
                  </div>
                  <div className={`text-[11px] mt-0.5 ${isPro ? 'text-amber-300/50' : 'text-white/30'}`}>{MODE_I18N[m.id] ? (t as any)[MODE_I18N[m.id][1]] : m.description}</div>
                </div>
              </button>
            ); })}
          </div>
        ...
        {/* Topic */}
        <div>
          <h3 className="text-white/60 text-xs font-bold uppercase tracking-wider mb-3">{t.topicLabel}</h3>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 animate-fadeIn" onClick={() => setModal(null)}>
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-slate-900 rounded-t-3xl p-5 pb-10 max-h-[70vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-white font-bold text-lg">{t.selectChar1} {modal}</h3>
